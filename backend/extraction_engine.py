...
# 8000 serves the shared general model (drafting, buckets, translation). 8091 serves the
# fine-tuned extraction model, which only returns extraction JSON and so must not replace 8000.
EXTRACTION_PORTS = (8000, 8091)


def local_model_url(value, ports=(8000,)):
    parsed = urlsplit(value)
    if (parsed.scheme != 'http' or parsed.hostname not in {'127.0.0.1', 'localhost', '::1'}
            or parsed.port not in ports or parsed.username or parsed.password
            or parsed.query or parsed.fragment or parsed.path.rstrip('/') != '/v1'):
        raise ValueError('The model must use loopback HTTP on an approved port ('
                         + ', '.join(map(str, ports)) + ') at /v1.')
    # Avoid hostname resolution for localhost and keep the approved destination exact.
    host = '[::1]' if parsed.hostname == '::1' else '127.0.0.1'
    return 'http://' + host + ':' + str(parsed.port) + '/v1'
...
class LocalTransport:
    ...
    def __init__(self, base_url, opener=None, ports=(8000,)):
        self.base_url = local_model_url(base_url, ports)
...
class LocalEngine:
    def __init__(self, model_dir=None, base_url=None, transport=None):
        ...
        # AFTERWORD_EXTRACT_URL points extraction at the fine-tuned model (e.g. :8091) without
        # moving the shared general model that AFTERWORD_LLM_URL / port 8000 serves.
        configured = (base_url or os.environ.get('AFTERWORD_EXTRACT_URL')
                      or os.environ.get('AFTERWORD_LLM_URL', 'http://127.0.0.1:8000/v1'))
        self.base_url = local_model_url(configured, EXTRACTION_PORTS)
        self.transport = transport or LocalTransport(self.base_url, ports=EXTRACTION_PORTS)
        self.module = None
