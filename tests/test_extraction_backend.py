...
def test_authoritative_files_are_unchanged():
    hashes = {'engine.py': 'a8e4f92b2378b0e670fbdd68df61c36a66bbbe1f148b3ecb0471be5941a82c75',
...
def test_engine_postprocessing_error_is_a_failure_not_an_http_crash(tmp_path):
    transport = Transport({'cat': 'bank', 'amt': 12.99, 'ev': 'not a list'})
...
def test_malformed_model_value_is_reported_and_removed_not_accepted(tmp_path):
    transport = Transport({'cat': 'bank', 'amt': 'not a number', 'ev': [1]})
    result = client_for(service(tmp_path, transport)).post('/extract', json=item()).json()
    assert result['status'] == 'needs_review'
    assert "'amt' not a number" in result['checks']['schema_errors']
    assert 'amt' not in result['finding']
...
def test_extraction_may_use_the_fine_tuned_port_but_nothing_else_may(monkeypatch):
    from backend.extraction_engine import EXTRACTION_PORTS
    assert local_model_url('http://localhost:8091/v1', EXTRACTION_PORTS) == 'http://127.0.0.1:8091/v1'
    with pytest.raises(ValueError):
        local_model_url('http://127.0.0.1:8091/v1')            # default boundary stays port 8000 only
    with pytest.raises(ValueError):
        local_model_url('http://127.0.0.1:8090/v1', EXTRACTION_PORTS)
    monkeypatch.setenv('AFTERWORD_EXTRACT_URL', 'http://127.0.0.1:8091/v1')
    target = LocalEngine(ROOT / 'model', transport=Transport())
    assert target.base_url == 'http://127.0.0.1:8091/v1'


def test_engine_adds_tags_rank_fields_and_gate_checks(tmp_path):
    result = service(tmp_path).extract(item())
    finding = result['finding']
    assert 'recurring_drain' in finding['tags'] and finding['priority'] in {'P1', 'P2', 'P3'}
    assert isinstance(finding['priority_score'], int) and finding['priority_reasons']
    assert result['checks']['gate'].startswith('off')        # fake transport returns no token probabilities


def test_contacts_are_read_from_the_document_not_invented(tmp_path):
    text = 'Monthly charge: $12.99\nVisit www.example-bank.com/estates or call (800) 555-0142.\nEmail help@example-bank.com'
    finding = service(tmp_path).extract(item(text=text))['finding']
    assert finding['contacts'] == {'urls': ['www.example-bank.com/estates'], 'emails': ['help@example-bank.com'],
                                   'phones': ['(800) 555-0142']}
    plain = service(tmp_path / 'b').extract(item(identifier='l2', text='Monthly charge: $12.99\nAccount 90213'))['finding']
    assert plain['contacts'] == {'urls': [], 'emails': [], 'phones': []}
