...
(() => {
  ...
  Object.assign(window.actions,{'outreach-add-scan':()=>openUpload(),'outreach-resume-scan':el=>resumeScan(el.dataset.id),'scan-documents':()=>{closeDialog();go('documents');},'scan-letter':el=>{closeDialog();O().openProviderLetter(el.dataset.provider,el.dataset.finding);}});
