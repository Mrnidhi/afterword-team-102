...
(() => {
  ...
  function showReview(){
    ...
    bind('scan-extract',async()=>{
      ...
      modal(contacts.length?'Contacts added from your scanned letter':'Scan reviewed; no contact found',`<p>${contacts.length?'The local model proposed these contacts. Every stored contact matched the reviewed text.':'No contact detail passed the source check. No address was invented or added. You can use a clearer scan or enter an address you have verified.'}</p>${contactHTML}<p class="fine">Processed by local OCR and the configured local contact-reading model. This is source matching, not independent proof that an address is current.</p>`,button('Back to documents','scan-documents')+actionButton('scan-add-another','Add another scan'));
  ...
  Object.assign(window.actions,{'outreach-add-scan':()=>openUpload(),'outreach-resume-scan':el=>resumeScan(el.dataset.id),'scan-documents':()=>{closeDialog();go('documents');}});
