...
(() => {
  ...
  let providers=[], archive={findings:[],documents:[]}, service=null, connection='checking', loadError='', activeFinding='insurance', activeTemplate=C.defaults.insurance;
  ...
  function providerEvidence(p) {
    ...
    const company=providers.find(item=>item.provider_id===p.provider_id)?.display_name;
    const evidence=(p.evidence||[]).map(e=>{
      ...
      return `<div class="outreach-evidence"><span>${esc(e.doc_id||e.document_id||'Offline directory')}${Number.isInteger(e.start)?' · characters '+e.start+'–'+e.end:''}</span>${e.quote?`<blockquote>${esc(e.quote)}</blockquote>`:''}</div>`;
    }).join('');
  ...
  function localResolve(id) {
    ...
    for(const p of providers.filter(p=>wanted.includes(p.provider_id))) {
      ...
      const address=saved.config.confirmed_control?C.demoAddress(saved.config.demo_mailbox,p.demo_alias):'';
  ...
  function lettersView() {
    const d=draft(),r=resolved[activeFinding],check=C.preflight(d,{demoMailbox:saved.config.confirmed_control?saved.config.demo_mailbox:'',verifiedDomains:saved.verifiedDomains});
  ...
  function reviewStillValid() {
    ...
    const latest=C.preflight(draft(),{demoMailbox:saved.config.confirmed_control?saved.config.demo_mailbox:'',verifiedDomains:saved.verifiedDomains});
  ...
  async function initialize(force=false) {
    connection='checking';service=null;loadError='';
    ...
    try {
      ...
      if(service){
        const results=await Promise.allSettled([api('/providers'),api('/outreach')]);
  ...
  window.AfterwordOutreach={api,getService:()=>service,currentDraft:()=>draft(),refreshContacts:()=>resolve(activeFinding,true),storeDraft,showReview,reviewError,log,refreshPrivacy:loadPrivacy,rememberServerConsent};
