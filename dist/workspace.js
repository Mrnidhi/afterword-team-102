...
window.prepareRoute = () => {
  ...
  if(![...nav.map(n=>n[0]),'ask','design'].includes(state.route))state.route='overview';
  if (previousRouteHash===hash) return;
  previousRouteHash=hash;
  const [route,query='']=hash.split('?');
  state.route=[...nav.map(n=>n[0]),'ask','design'].includes(route)?route:'overview';
...
function selectLetter(id) {
  ...
  letterType=id;letterDraft=sessionDrafts[id]||state.drafts[id]||null;letterPreview=false;
  go('letters?template='+id);
}
const originalLettersPage=lettersPage;
function completeLettersPage() {
  return originalLettersPage().replace('<div class="letter-guidance">',`<div class="saved-draft-list"><span class="eyebrow">SAVED IN THIS BROWSER</span><strong>${Object.keys(state.drafts).length} of 3 drafts</strong><p>Each template keeps its own saved version.</p></div><div class="letter-guidance">`)+`<div class="letter-extra-actions"><p>Save keeps this version for your next visit. Preview, print or download before sharing it yourself.</p><div class="button-row">${button('Reset this template','reset-letter',false,'refresh')}${button('Print letter','print-letter',false,'files')}</div></div>`;
}

const routingRows = [
  {id:'policy',tier:'02',reason:'Historical beneficiary information needs comparison with the family’s estate record.',result:'Human confirmation needed'},
  {id:'will',tier:'02',reason:'Estate residue and a separately designated benefit must remain distinct.',result:'Human confirmation needed'},
  {id:'storage',tier:'01',reason:'A renewal amount and collection instruction can be organized from the text.',result:'Action candidate'},
  {id:'statement',tier:'00',reason:'The sample CSV amounts can be summed deterministically.',result:'$55.48 in sample rows'},
  {id:'medical',tier:'02',reason:'An invoice and a later receipt require a cross-record comparison.',result:'Payment allocation unknown'},
  {id:'receipt',tier:'02',reason:'The payment reference does not establish which invoice it settled.',result:'Provider confirmation needed'},
  {id:'voice',tier:'01',reason:'The transcript contains a lead about a location, not proof of ownership.',result:'Unverified asset lead'},
  {id:'pension',tier:'00',reason:'An acknowledgement can be displayed without a language model.',result:'Notification acknowledged'}
];
const tiers=[{id:'00',title:'Read & calculate',short:'Rules & parsers',desc:'Native text, structured rows and deterministic calculations. No model needed.'},{id:'01',title:'Organize a record',short:'Small local model',desc:'Proposed 8B model extracts candidate details with source references.'},{id:'02',title:'Compare the evidence',short:'Large local model',desc:'Proposed 70B model examines difficult relationships and keeps uncertainty visible.'},{id:'03',title:'Ask beyond the archive',short:'Optional external help',desc:'A proposed, general question is reviewed before any external request. Disabled in this demo.'}];
function ledgerRows() {
  const rows=routingRows.filter(r=>ledgerTier==='all'||r.tier===ledgerTier);
  return rows.length?rows.map(r=>{const d=records.find(d=>d.id===r.id);return `<button class="ledger-row" data-action="routing-detail" data-id="${r.id}"><span class="tier-index">${r.tier}</span><span><strong>${d.title}</strong><small>${d.type} · ${d.date}</small></span><span>${tiers.find(t=>t.id===r.tier).short}</span><span class="pill neutral">Design example</span>${icon('chevron')}</button>`;}).join(''):`<div class="empty-state">${icon('shield')}<h2>No external requests.</h2><p>The demo has no cloud processing. Tier 03 illustrates a future option requiring payload review.</p><a class="button" href="#exposure">See the exposure report</a></div>`;
}
function ledgerPage() {
  return heading('PROCESSING LEDGER','See the path behind a finding.','Explore why each sample record would use a different level of processing.',button('Export routing map','export-ledger',false,'download'))+`<div class="notice-strip">${icon('info')}<p><strong>Illustrative routing, not an execution log.</strong> No document has been processed by a model. Timing, cost and device measurements are pending.</p></div><div class="tier-overview">${tiers.map(t=>`<button class="tier-tile ${ledgerTier===t.id?'selected':''}" data-action="tier-filter" data-tier="${t.id}" aria-pressed="${ledgerTier===t.id}"><span>TIER ${t.id}</span><strong>${t.short}</strong><small>${routingRows.filter(r=>r.tier===t.id).length} sample routes ${icon('arrow')}</small></button>`).join('')}</div><section class="ledger-table"><div class="section-title"><h2>Sample routing decisions</h2><button class="text-link" data-action="tier-filter" data-tier="all">Show all tiers</button></div><div id="ledger-results" aria-live="polite">${ledgerRows()}</div></section><section class="local-activity"><div class="section-title"><h2>Your workspace activity</h2><span class="pill neutral">This browser</span></div>${state.activity.length?state.activity.slice(0,12).map(a=>`<div class="local-event">${icon('clock')}<span>${escapeHTML(a.text)}</span><time>${escapeHTML(formatTime(a.at))}</time></div>`).join(''):`<div class="inline-empty">${icon('clock')}<div><strong>A fresh workspace.</strong><p>Saved notes, staged files and completed actions will appear here.</p></div></div>`}<p class="fine">Activity is editable browser state. It is not a security or compliance audit log.</p></section>`;
}
...
function exposurePage() {
  return heading('EXPOSURE REPORT','Personal stays personal. By design.','Inspect the boundary between your archive and a proposed external question.',button('Export report','export-exposure',false,'download'))+`<section class="boundary-hero"><div><span class="eyebrow">THIS DEMONSTRATION</span><h2>No archive sent.<br>No cloud AI connected.</h2><p>The categories below come from fictional records. They illustrate the product’s intended controls, not a measured privacy guarantee.</p><a href="#privacy" class="text-link">Explore the HP ZGX architecture ${icon('arrow')}</a></div><div class="boundary-emblem">${icon('shield')}<span>LOCAL FIRST</span><strong>Your archive.<br>Your decision.</strong></div></section><div class="section-title"><h2>What the sample archive contains</h2><span class="fine">Manually defined sample categories</span></div><div class="exposure-categories">${exposureGroups.map(g=>`<article><span class="category-icon">${icon(g.icon)}</span><h3>${g.title}</h3><p>${g.details}</p><div>${g.sources.map(id=>`<button class="source-chip" data-action="document" data-id="${id}">${records.find(d=>d.id===id).title} ${icon('arrow')}</button>`).join('')}</div></article>`).join('')}</div><section class="payload-section"><div class="section-title"><div><div class="eyebrow">OPTIONAL TIER 03 / DESIGN EXAMPLE</div><h2>Review exactly what would leave.</h2></div><span class="pill amber">External processing unavailable</span></div><div class="payload-grid"><article><span class="payload-label">STAYS IN THE ARCHIVE</span><h3>Original sample context</h3><p>Arun Rao’s 2019 Cedar Life letter names Maya Rao and mentions $250,000. A family copy of a will names Priya Rao for the estate residue.</p><div class="payload-fields"><span>Personal names</span><span>Provider</span><span>Amount</span><span>Source documents</span></div></article><article class="proposed-payload"><span class="payload-label">PROPOSED GENERAL QUESTION</span><h3>A smaller, explicit request</h3><blockquote>What information might an insurer request before discussing a deceased policyholder’s account?</blockquote><p>No family names, amounts, provider names or source excerpts are included in this example.</p><button class="button" data-action="inspect-payload">${icon('eye')}Inspect request preview</button></article></div><p class="fine">Generalization alone does not guarantee privacy. A production system needs policy enforcement, leakage evaluation, explicit consent and a record of the actual request. Nothing is sent from this screen.</p></section>`;
}
function stationPage() {
  return heading('AFTERWORD / HP ZGX NANO','A private workspace. A powerful foundation.','The proposed local AI architecture behind the family experience.')+`<section class="station-hero"><div><span class="dark-tag">TARGET PLATFORM · NOT CONNECTED</span><h2>Intelligence close<br>to what matters.</h2><p>Organize deeply personal records with local parsing and inference, escalating only when the task requires it.</p><div class="station-model">HP ZGX Nano G1n <span>AI Station</span></div><a class="button" href="https://www.hp.com/us-en/workstations/zgx-nano-ai-station.html" target="_blank" rel="noopener noreferrer">Explore HP ZGX ${icon('external')}</a></div><figure><img src="assets/hp-zgx-nano.jpg" alt="Official HP product photograph of the HP ZGX Nano AI Station"><figcaption>Target hardware · official HP product imagery</figcaption></figure></section><div class="station-status"><div><span class="status-dot"></span><strong>Device not connected</strong><p>No hardware telemetry or model service is available in this browser demo.</p></div>${button('View connection requirements','connection-help',false,'server')}</div><div class="section-title"><div><div class="eyebrow">USE THE RIGHT LEVEL OF INTELLIGENCE</div><h2>Four tiers. One transparent workflow.</h2></div><a class="text-link" href="#ledger">Explore the routing ledger ${icon('arrow')}</a></div><div class="architecture-grid">${tiers.map(t=>`<article><span class="tier-index">${t.id}</span><h3>${t.title}</h3><span class="pill neutral">${t.short}</span><p>${t.desc}</p></article>`).join('')}</div><section class="station-bottom"><div><h2>Review belongs to you.</h2><p>Every proposed finding returns to a person with its sources, unresolved questions and a reversible next step. Drafts are prepared here; messages are sent by you.</p><a class="text-link" href="#evidence">See evidence review ${icon('arrow')}</a></div><div><h2>Measure before promising.</h2><p>The 8B and 70B sizes are proposed targets. Speed, memory use, accuracy, energy use and cloud savings must be measured on the actual workstation.</p><span class="pill neutral">Benchmarks pending</span></div></section>`;
}
function settingsPage() {
  return heading('WORKSPACE SETTINGS','Make this space yours.','Reading preferences, local data and a clear view of what is connected.')+`<div class="settings-layout"><section class="settings-panel"><div class="section-title"><h2>Your sample workspace</h2><span class="avatar">AR</span></div><dl class="settings-facts"><div><dt>Archive</dt><dd>Arun Rao · fictional</dd></div><div><dt>Organizer</dt><dd>Priya Rao · fictional</dd></div><div><dt>Saved drafts</dt><dd>${Object.keys(state.drafts).length} templates</dd></div><div><dt>File queue</dt><dd>${state.staged.length} file names · no contents</dd></div><div><dt>Storage</dt><dd>Browser local storage · unencrypted</dd></div></dl><div class="setting-action"><div><h3>Reading size</h3><p>${state.largeText?'Larger':'Standard'} text. Motion follows your device preference.</p></div>${button('Change','preferences')}</div><div class="setting-action"><div><h3>Keep a copy</h3><p>Export notes, reminders, draft text and demo preferences as JSON.</p></div>${button('Export workspace','export-workspace',false,'download')}</div><div class="setting-action"><div><h3>Start fresh</h3><p>Restore the fictional workspace. Export first if you want to keep your edits.</p></div><button class="button danger" data-action="reset-workspace">Reset demo</button></div>${resetBackup?`<div class="notice-strip">${icon('refresh')}<p>Your previous workspace can be restored until you reload this page.</p>${button('Undo reset','undo-reset')}</div>`:''}</section><aside><section class="settings-panel"><span class="eyebrow">CONNECTIONS</span><h2>Everything accounted for.</h2>${[['HP ZGX station','Not connected'],['Local AI models','Not connected'],['Email and bank accounts','Not connected'],['Cloud model service','Not connected'],['Static frontend','Available']].map(([name,status])=>`<div class="connection-row"><span>${name}</span><strong>${status}</strong></div>`).join('')}<p class="fine">The website loads its typography from Google Fonts. It makes no AI requests and has no analytics or account integration.</p></section><section class="settings-panel"><h2>About this edition</h2><p>Afterword is an independent Team 102 concept for HP ZGX. It is not an HP product or an endorsed service.</p><a class="text-link" href="#design">Design & research ${icon('arrow')}</a></section></aside></div>`;
}

Object.assign(window.views,{plan:completePlanPage,documents:completeDocumentsPage,evidence:completeEvidencePage,letters:completeLettersPage,exposure:exposurePage,ledger:ledgerPage,privacy:stationPage,settings:settingsPage});
Object.assign(window.actions,{
  ...
  'export-plan':()=>exportFile('afterword-action-plan.csv',toCSV([['Fictional action','Status','Category','Date or reminder','Source','Historical amount mentioned','Your note'],...filteredTasks().map(t=>[t.title,statusNames[status(t)],t.category,state.reminders[t.id]||t.date,t.source,t.value,state.taskNotes[t.id]||''])]),'text/csv'),
  'document-back':()=>openTask(taskReturn),
  'task-evidence':()=>{const id=state.activeTask;closeDialog();go('evidence?finding='+id);},
  'sample-import':()=>{if(state.imported)return;state.imported=true;recordActivity('Added the fictional family letter');saveNotice('Fictional letter added to your sample library.');closeDialog();go('documents');},
  'remove-queued':a=>{importQueue.splice(Number(a.dataset.index),1);showImport();},
  'stage-files':()=>{if(!importQueue.length)return;const count=importQueue.length;state.staged.push(...importQueue.map(f=>({...f,id:'local-'+crypto.randomUUID(),added:new Date().toISOString()})));importQueue=[];recordActivity(`Staged ${count} file name${count===1?'':'s'}; no file content read`);saveNotice('File names staged. A connected service is needed to process their contents.');closeDialog();go('documents');},
  ...
  'tier-filter':a=>{ledgerTier=a.dataset.tier;render();},
  'routing-detail':a=>{const row=routingRows.find(r=>r.id===a.dataset.id);if(!row)return;const d=records.find(d=>d.id===row.id);modal('Why this tier?',`<span class="pill blue">TIER ${row.tier} · ${tiers.find(t=>t.id===row.tier).short}</span><h3 class="dialog-heading">${d.title}</h3><p>${row.reason}</p><div class="subtle-box"><strong>Illustrative result</strong><p>${row.result}</p></div>${documentLink(d.id)}<p class="fine">This is a hand-authored routing example. No inference run, latency or confidence measurement is associated with it.</p>`,button('Back to ledger','close-modal'));},
  'export-ledger':()=>exportFile('afterword-routing-design.csv',toCSV([['Sample record','Proposed tier','Routing reason','Illustrative result','Execution status'],...routingRows.map(r=>[records.find(d=>d.id===r.id).title,r.tier,r.reason,r.result,'Not executed'])]),'text/csv'),
  ...
  'confirm-reset':()=>{resetBackup=AfterwordStore.clean(state);Object.assign(state,AfterwordStore.defaults());for(const cache of [taskWorking,reviewWorking])for(const key of Object.keys(cache))delete cache[key];for(const key of Object.keys(sessionDrafts))delete sessionDrafts[key];letterDraft=null;letterPreview=false;conversation=[];importQueue=[];state.memoryFilter='all';saveNotice('Demo restored. Undo is available in Workspace settings.');closeDialog();render();},
  'undo-reset':()=>{if(!resetBackup)return;Object.assign(state,resetBackup);resetBackup=null;for(const cache of [sessionDrafts,taskWorking,reviewWorking])for(const key of Object.keys(cache))delete cache[key];letterDraft=state.drafts[letterType]||null;saveNotice('Previous workspace restored.');render();},
