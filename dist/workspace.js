...
afterRender=()=>{previousAfterRender();if(state.storageAvailable===false)$('#main').insertAdjacentHTML('afterbegin','<div class="notice-strip storage-warning" role="alert"><p><strong>Your private workspace is unavailable.</strong> Reconnect to the HP system before making changes so nothing is lost.</p><a class="text-link" href="#settings">Workspace settings</a></div>');if(state.linkedDocument){const id=state.linkedDocument;state.linkedDocument=null;openDocument(id);}if(state.route==='overview'){const activity=$('.activity-list');if(activity&&state.activity.length)activity.innerHTML=state.activity.slice(0,3).map(a=>`<div class="activity"><span class="activity-icon">${icon('clock')}</span><div><p>${escapeHTML(a.text)}</p><small>${escapeHTML(formatTime(a.at))}</small></div></div>`).join('');}};
render();
