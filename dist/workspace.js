...
window.prepareRoute = () => {
  const rawHash=location.hash.slice(1)||'landing';
  const hash=rawHash==='privacy'?'exposure':rawHash==='design'?'settings':rawHash;
  const [route,query='']=hash.split('?');
  state.route=['landing',...nav.map(n=>n[0]),'ask'].includes(route)?route:'overview';
...
function settingsPage() {
  ...
  return heading('WORKSPACE SETTINGS','Make this space yours.','Reading preferences, local data and a clear view of what is connected.')+`<div class="settings-layout"><section class="settings-panel"><div class="section-title"><h2>Your sample workspace</h2><span class="avatar">AR</span></div><dl class="settings-facts"><div><dt>Archive</dt><dd>Arun Rao · fictional</dd></div><div><dt>Organizer</dt><dd>Priya Rao · fictional</dd></div><div><dt>Staged file names</dt><dd>${state.staged.length} names only · file contents not included</dd></div><div><dt>Browser notes</dt><dd>Local browser storage · unencrypted</dd></div><div><dt>Local record archive</dt><dd>Text records and scans are stored by the local service when uploaded.</dd></div></dl><div class="setting-action"><div><h3>Appearance</h3><p>${state.largeText?'Larger':'Standard'} text · ${state.ambientMotion?'gentle background motion':'still background'}.</p></div>${button('Change','preferences')}</div><div class="setting-action"><div><h3>Keep a copy</h3><p>Export your notes, reminders, progress and demo preferences as JSON.</p></div>${button('Export workspace','export-workspace',false,'download')}</div><div class="setting-action"><div><h3>Start fresh</h3><p>Restore the fictional workspace. Export first if you want to keep your edits.</p></div><button class="button danger" data-action="reset-workspace">Reset demo</button></div>${resetBackup?`<div class="notice-strip">${icon('refresh')}<p>Your previous workspace can be restored until you reload this page.</p>${button('Undo reset','undo-reset')}</div>`:''}</section><aside><section class="settings-panel"><span class="eyebrow">CONNECTIONS</span><h2>Connection status</h2><div class="connection-row"><span>Local record intake</span><strong>Browser-local</strong></div><p class="fine">The website loads its typography from Google Fonts. It makes no AI requests and has no analytics or account integration.</p></section><section class="settings-panel"><h2>About this edition</h2><p>A fictional family case for trying the workflow. Documents, names, providers and amounts are examples.</p><button class="text-link" data-action="workspace-info">How to use this workspace ${icon('arrow')}</button></section></aside></div>`;
}

Object.assign(window.views,{plan:completePlanPage,documents:completeDocumentsPage,evidence:completeEvidencePage,exposure:exposurePage,ledger:ledgerPage,settings:settingsPage});
Object.assign(window.actions,{
  ...
  'export-ledger':()=>exportFile('afterword-workspace-activity.csv',toCSV([['Update','Time'],...state.activity.map(a=>[a.text,a.at])]),'text/csv'),
  ...
  'confirm-reset':()=>{resetBackup=AfterwordStore.clean(state);Object.assign(state,AfterwordStore.defaults());for(const cache of [taskWorking,reviewWorking])for(const key of Object.keys(cache))delete cache[key];conversation=[];importQueue=[];planSearch='';planCategory='all';planSort='suggested';ledgerTier='all';state.filter='all';state.docFilter='all';state.query='';state.memoryFilter='all';saveNotice('Demo restored. Undo is available in Workspace settings.');closeDialog();render();},
  'undo-reset':()=>{if(!resetBackup)return;Object.assign(state,resetBackup);resetBackup=null;for(const cache of [taskWorking,reviewWorking])for(const key of Object.keys(cache))delete cache[key];saveNotice('Previous workspace restored.');render();},
